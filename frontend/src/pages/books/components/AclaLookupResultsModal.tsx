...
import type { AclaLookupResultDto } from '@/api/acla-lookup'
import { PiCheckCircle, PiXCircle } from 'react-icons/pi'

interface AclaLookupResultsModalProps {
  isOpen: boolean
  onClose: () => void
  results: AclaLookupResultDto[]
}
...
export function AclaLookupResultsModal({ isOpen, onClose, results }: AclaLookupResultsModalProps) {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="ACLA Availability Lookup Results"
      size="lg"
      footer={
        <div className="flex justify-end">
          <Button variant="primary" onClick={onClose} data-test="close-acla-lookup-results">
            Close
          </Button>
        </div>
      }
