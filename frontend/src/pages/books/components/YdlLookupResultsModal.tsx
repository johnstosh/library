...
import type { YdlLookupResultDto } from '@/api/ydl-lookup'
import { PiCheckCircle, PiXCircle } from 'react-icons/pi'

interface YdlLookupResultsModalProps {
  isOpen: boolean
  onClose: () => void
  results: YdlLookupResultDto[]
}
...
export function YdlLookupResultsModal({ isOpen, onClose, results }: YdlLookupResultsModalProps) {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="YDL Availability Lookup Results"
      size="lg"
      footer={
        <div className="flex justify-end">
          <Button variant="primary" onClick={onClose} data-test="close-ydl-lookup-results">
            Close
          </Button>
        </div>
      }
