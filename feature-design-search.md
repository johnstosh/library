...
- `status` (string, optional, multi-value) - Limit books to those matching any listed status-filter key (`in-library`, `electronic-resource`, `without-loc`, `lost`, `withdrawn`, `on-order`, `requested`). `in-library`, `electronic-resource`, and `without-loc` are Active-only (`without-loc` also excludes electronic resources). When omitted, WITHDRAWN and REQUESTED stay hidden. When every key is selected, status is unconstrained. See issue #337.
...
| Without LOC (status) | `status = ACTIVE AND (locNumber IS NULL OR locNumber = '') AND (electronicResource IS NULL OR electronicResource = false)` |
...
- `status` (string, optional) - Comma-separated status-filter keys (OR with each other; in-library, electronic-resource, and without-loc are Active-only; without-loc additionally excludes electronic resources). Selecting every key leaves status unconstrained. Legacy `inLib`/`elec`/`withoutLoc`/`requestedStatus`/`notActiveStatus` still read. See issue #337.
